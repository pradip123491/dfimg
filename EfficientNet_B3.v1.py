#EfficientNet (B3) using timm with CHUNK-wise loading
import os
import cv2
import torch
import numpy as np
...
IMAGE_SIZE = 512
BATCH_SIZE = 16
CHUNK_SIZE = 15000  # Max number of images to load per chunk
EPOCHS = 20
LEARNING_RATE = 5e-5
CHECKPOINT_DIR = '/home/shavak/YC/project/models/checkpoints_efficientnet_v1.0'
...
# ──────────────────────────────────────────────────
# 🧠 Lazy Dataset Loader with chunk logic
# ──────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────
model = create_model('efficientnet_b3', pretrained=True, num_classes=1)
    ...
    for epoch in range(start_epoch, EPOCHS):
        try:
            ...
            for imgs, labels in pbar:
                imgs, labels = imgs.to(DEVICE, non_blocking=True), torch.tensor(labels).float().unsqueeze(1).to(DEVICE, non_blocking=True)
                optimizer.zero_grad()
                outputs = model(imgs)
            ...
            with torch.no_grad():
                for imgs, labels in val_loader:
                    imgs, labels = imgs.to(DEVICE, non_blocking=True), torch.tensor(labels).to(DEVICE, non_blocking=True)
                    outputs = model(imgs)
                    preds = (torch.sigmoid(outputs).squeeze() > 0.5).int()
            ...
            log_str = f"📊 Epoch {epoch+1} → Train Acc: {correct/total:.4f}, Val Acc: {val_acc:.4f}\n"
            print(log_str)
            ...
            torch.save({
                ...
                'val_acc': val_acc
            }, os.path.join(CHECKPOINT_DIR, f'effnet_epoch_{epoch + 1}_v1.0.pth'))
            ...
            if val_acc > best_val_acc:
                ...
                torch.save({
                    ...
                    'optimizer_state': optimizer.state_dict(),
                    'val_acc': val_acc
                }, os.path.join(CHECKPOINT_DIR, 'best_efficientnet_v1.0.pth'))
            else:
                trigger_times += 1
